...
int main(void)
{
  ...
  rtmSetErrorStatus(TempControl_OpenLoop_5_6_M, 0);
  ...
  TempControl_OpenLoop_5_6_initialize();
  ;
  ;

  /* External Mode initialization */
  errorCode = extmodeInit(TempControl_OpenLoop_5_6_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TempControl_OpenLoop_5_6_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(TempControl_OpenLoop_5_6_M);
  __enable_irq();
  ;
  while (runModel) {
  ...
  /* Terminate model */
  TempControl_OpenLoop_5_6_terminate();
